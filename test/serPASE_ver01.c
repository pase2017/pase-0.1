...
//   int time_index; 
   ...
   /* Parse command line */
   {
	 ...
	 else if ( strcmp(argv[arg_index], "-maxLevels") == 0 )
	 {
	    arg_index++;
...
//	 hypre_ClearTiming();
	 ...
	 /* eigenvectors - create a multivector */
	 eigenvectors = 
	    mv_MultiVectorCreateFromSampleVector(interpreter, blockSize, U_array[level]);
	 ...
	 for (idx_eig = 0; idx_eig < blockSize; ++idx_eig)
	 {
	    if ( level == num_levels-2 )
	    {
	       ...
	       /* Q pvx_Hh[0:N_H-1] + alpha[0] pvx_h[0] + ... + alpha[blockSize-1] pvx_h[blockSize-1] */
