...
//   HYPRE_BoomerAMGSetTol(amg_solver, 1e-7);             /* conv. tolerance */
   ...
   if (flag)
   {
	 ...
	 for ( idx_eig = 0; idx_eig < block_size; ++idx_eig)
	 {
	    PASE_ParVectorGetParVector( Q_array[0], block_size, pvx_h, pvx_Hh[idx_eig], pvx[idx_eig] );
	 ...
	 cur_sum_eigenvalues = 0;
	 for ( idx_eig = 0; idx_eig < block_size; ++idx_eig)
	 {
	    cur_sum_eigenvalues += eigenvalues[idx_eig];
	 }

	 if (fabs(cur_sum_eigenvalues-pre_sum_eigenvalues)<1.e-8*block_size)
	 {
	    break;
	 }
	 else {
	    pre_sum_eigenvalues = cur_sum_eigenvalues;
	 }
      }
   }
   else 
   {
	 ...
	 printf ( "PCG solve A_hh U = lambda_Hh B_hh U_Hh\n" );
	 for ( idx_eig = 0; idx_eig < block_size; ++idx_eig)
	 {
