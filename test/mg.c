...
//   PASE_ParVectorPrint(b_Hh, "b_Hh");
   ...
   /*---------------------------- TEST PASE PCG without preocondition ----------------------------*/
   PASE_ParVectorSetConstantValues(x_Hh, 1.0);

   {
      ...
      /* Now setup and solve! */
      hypre_PCGSetup(solver, A_Hh, b_Hh, x_Hh);
      hypre_PCGSolve(solver, A_Hh, b_Hh, x_Hh);
...
//   PASE_ParVectorPrint(x_Hh, "pcg_Hh");
//   PASE_ParVectorPrint(b_Hh, "b_Hh");
   {
       HYPRE_Solver mg_solver;
       HYPRE_Int num_level = 2;
       PASE_ParCSRMGCreate(&mg_solver);
       PASE_MGAddLevel(mg_solver, A_array, NULL, P_array, num_level);
       PASE_ParCSRMGInit( MPI_COMM_WORLD, mg_solver, NULL, 1, 1); 
       PASE_ParCSRMGSolve(mg_solver);
       PASE_ParCSRMGDestroy(mg_solver);
   }

   PASE_ParCSRMatrixMatvec(1.0, A_Hh, x_Hh, -1.0, b_Hh);
   HYPRE_Real prod;
   PASE_ParVectorInnerProd(b_Hh, b_Hh, &prod);
   printf ( "prod = %f\n", prod );
