/*
   ...
   Sample run:   parPASE_ver01 -block_size 20 -n 100 -max_leveles 6 
...
//   int time_index; 
   ...
   /* Parse command line */
   {
	 ...
	 else if ( strcmp(argv[arg_index], "-max_leveles") == 0 )
	 {
	    arg_index++;
	    max_leveles = atoi(argv[arg_index++]);
	 }
	 ...
	 printf("  -max_leveles <n>      : max levels of AMG (default: 5)\n");
	 printf("\n");
      }
   ...
   HYPRE_BoomerAMGSetMaxLevels(amg_solver, max_leveles);  /* maximum number of levels */
   ...
   for ( level = num_levels-2; level >= 0; --level )
   {
	 ...
	 if (level < num_levels-3)
	 {
	    ...
	    hypre_TFree(tmp);
	    /* TODO:
	     * 按照下面的注释修改后, 这里就不用再释放了 */
	    PASE_ParCSRMatrixDestroy(parcsr_A_Hh);
	    PASE_ParCSRMatrixDestroy(parcsr_B_Hh);
	 }

	 /* TODO:
	  * 这里最好改成在level == num_levels-3时创建, 在level < num_levels只改变aux_Hh的值 
	  * 需要传入P A_h 以及u_h, 即重新计算aux_hH和aux_hh */
	 ...
	 /* TODO:
	  * 这是是否可以只在num_levels-2和num_levels-3创建*/
	 if (level == num_levels-2)
	 {
...
//	 hypre_ClearTiming();
	 ...
	 /* TODO:
	  * 创建好像与矩阵无关, 也可以放在循环外面, precond好像也是 */
	 /* Create solver */
	 ...
	 /* TODO:
	  * 以上都可以放在循环外面, Setup会判断pcg_solver中的向量存储指针是否为空, 
	  * 若为空则删除, 所以可以将上述都放在循环外 */
