...
PetscErrorCode GCG_Eigen(Mat A, Mat B, PetscReal *eval, Vec *evec, PetscInt nev, PetscReal abs_tol, PetscReal cg_tol, PetscInt nsmooth, PetscInt max_iter)
{
    //--------------------定义变量--------------------------------------------
    ...
    //ierr = ReadVec(evec, nev);
    //ierr = WriteVec(V, nev, "../mat/guangji/Init_guangji_nev1.txt");CHKERRQ(ierr);
    //对初值做一次B正交化,暂时默认初值不会有线性相关的向量，即nev不会被修改
    ierr = GCG_Orthogonal(V, B, 0, &dim_x, V_tmp, Orth_tmp, Ind, time+1);CHKERRQ(ierr);
    //计算得到小规模特征值计算的矩阵AA,BB并保存备份AAt
    ierr = RayleighRitz(A, V, AA, approx_eval, NULL, AA_copy, 0, 0, dim_x, V_tmp[0], NULL, time+2);CHKERRQ(ierr);
    //计算初始近似特征向量并正交化
    ...
    dim_xpw = 2*dim_x;
    ierr = GCG_Orthogonal(V, B, dim_x, &dim_xpw, V_tmp, Orth_tmp, Ind, time+1);CHKERRQ(ierr);
    ierr = RayleighRitz(A, V, AA, approx_eval, NULL, AA_copy, 0, 0, dim_xpw, V_tmp[0], NULL, time+2);CHKERRQ(ierr);
    ...
    while((nunlock > 0)&&(iter < max_iter))
    {
        ...
        ierr = GCG_Orthogonal(V, B, dim_xp, &dim_xpw, V_tmp, Orth_tmp, Ind, time+1);CHKERRQ(ierr);
        //PetscPrintf(PETSC_COMM_WORLD, "iter: %d, after  ortho, W:\n", iter);
        //ierr = PrintVec(V+dim_xp, dim_xpw-dim_xp, 10, 20);
        //计算小规模矩阵特征值
        ierr = RayleighRitz(A, V, AA, approx_eval, AA_sub, AA_copy, dim_xp, last_dim_xpw, dim_xpw, V_tmp[0], small_tmp, time+2);CHKERRQ(ierr);
        //检查特征值重数
        ...
        PetscPrintf(PETSC_COMM_WORLD,"'iter: %d, time: iter total: %lf, get w: %lf, orth: %lf, rayleigh-ritz: %lf, get x: %lf, get p: %lf'\n", 
                iter, (double)(titer2-titer1), time[0]-last_time[0], time[1]-last_time[1], time[2]-last_time[2], time[3]-last_time[3], time[4]-last_time[4]);
        fflush(stdout);
...
PetscErrorCode AllocateVecs(Mat A, Vec **evec, Vec **V_1, Vec **V_2, Vec **V_3, PetscInt nev, PetscInt a, PetscInt b, PetscInt c)
{
    ...
    ierr = VecDuplicateVecs((*V_1)[0],c,V_3);CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
{
    PetscInt       i, nunlocktmp = 0, print_eval;
...
PetscErrorCode RayleighRitz(Mat A, Vec *V, PetscReal *AA, PetscReal *approx_eval, PetscReal *AA_sub, PetscReal *AA_copy, PetscInt start, PetscInt last_dim, PetscInt dim, Vec tmp, PetscReal *small_tmp, PetscReal *time)
{
    PetscErrorCode ierr;
    PetscLogDouble t1, t2;
    ierr = PetscTime(&t1);CHKERRQ(ierr);
...
//全部正交化，则start=0
PetscErrorCode GCG_Orthogonal(Vec *V, Mat B, PetscInt start, PetscInt *end, Vec *V_tmp, Vec *Nonzero_Vec, PetscInt *Ind, PetscReal *time)
{
    PetscErrorCode ierr;
    PetscLogDouble t1, t2;
    ierr = PetscTime(&t1);CHKERRQ(ierr);
...
void OrthogonalSmall(PetscReal *V, PetscReal **B, PetscInt dim_xpw, PetscInt dim_x, PetscInt *dim_xp, PetscInt *Ind)
{
    ...
    if(n_nonzero < (*dim_xp-dim_x))
    {
        *dim_xp = dim_x+n_nonzero;
        for( i=0; i<n_nonzero; i++ )
            memcpy(V+i*dim_xpw, V+Ind[i]*dim_xpw, dim_xpw*sizeof(PetscReal));
    }
}
