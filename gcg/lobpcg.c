...
{
  /* computes the matrix xy = x'*y stored in fortran style: gh is the leading dimension,
     height the number of rows and width the number of columns (cf. blas or lapack) */
...
lobpcg_MultiVectorByMatrix(
      ...
      mv_MultiVectorPtr y )
{
  /* computes y = x*r, where r is stored in fortran style */
...
lobpcg_checkResiduals( 
      ...
      HYPRE_Int num_lock)
{
   /* activeMask[i] == 1 means that lambda[i] is not converged  */
   HYPRE_Int i, n;
              ...
              HYPRE_Int residualNormsHistory_gh )
{
   ...
   utilities_FortranMatrix*	lambdaAB;/* eigenvalues of gramA u = lambda gram B u */
   utilities_FortranMatrix*	lambdaX; /* first sizeX eigenvalues in lambdaAB (ref) */
   ...
   *iterationNumber = 0;
   ...
   /* prepare to process eigenvalues history, if user has provided non-NULL as "lambdaHistory_values" a rgument */
   ...
   /* prepare to process residuals history, if user has provided non-NULL as "residualNormsHistory_values" argument */
   ...
   /* using num_lock to initializing activeMask */
   for ( i = 0; i < num_lock; i++ )
      activeMask[i] = 0;
   for ( i = num_lock; i < sizeX; i++ )
      activeMask[i] = 1;
   ...
   else 
   {
	 ...
	 {
	    utilities_FortranMatrixSelectBlock( lambdaHistory, 1, sizeX, 1, 1, historyColumn );
	    utilities_FortranMatrixCopy( lambda, 0, historyColumn );
	 }

	 if ( residualNormsHistory != NULL ) 
	 {
	    utilities_FortranMatrixSelectBlock( residualNormsHistory, 1, sizeX, 1, 1, historyColumn );
   ...
   for ( *iterationNumber = 1; exitFlag == 0 && *iterationNumber <= maxIterations; (*iterationNumber)++ ) 
   {

      sizeR = lobpcg_checkResiduals( residualNorms, lambda, tolerance, activeMaskm=, num_lock );
      if ( sizeR < 1 )
	 break;
	 ...
	 mv_MultiVectorCopy( blockVectorW, blockVectorBR );
      ...
      /* AR = A*R */
      operatorA( operatorAData, mv_MultiVectorGetData(blockVectorR), mv_MultiVectorGetData(blockVectorAR) );
	 ...
	 exitFlag = lobpcg_MultiVectorImplicitQR( blockVectorP, blockVectorBP, upperR, blockVectorW,blap_fn.dpotrf );
	 if ( exitFlag ) 
	 {
	 ...
	 lastP = lastR;
      ...
      utilities_FortranMatrixSelectBlock( gramA, 1, sizeX, 1, sizeX, gramXAX );
      utilities_FortranMatrixSelectBlock( gramA, firstR, lastR, 1, sizeX, gramRAX );
      utilities_FortranMatrixSelectBlock( gramA, firstR, lastR, firstR, lastR, gramRAR );

      utilities_FortranMatrixSelectBlock( gramB, 1, sizeX, 1, sizeX, gramXBX );
      utilities_FortranMatrixSelectBlock( gramB, firstR, lastR, 1, sizeX, gramRBX );
      utilities_FortranMatrixSelectBlock( gramB, firstR, lastR, firstR, lastR, gramRBR );
